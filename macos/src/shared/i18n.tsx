...
const translations: Record<string, Translation> = {
  ...
  "本功能会检查视频解析、下载及媒体处理所需组件。应用会优先复用设备上已有工具；缺失时仅安装应用专用副本，无需安装 Xcode、Node.js 或 Homebrew，也不会改动已有工具。": { "zh-Hant": "此功能會檢查影片解析、下載與媒體處理所需的元件。應用程式會優先使用裝置上現有的工具；如有缺少，僅會安裝應用程式專用副本，無需安裝 Xcode、Node.js 或 Homebrew，也不會改動現有工具。", en: "This checks the components required for video parsing, downloading, and media processing. Existing tools are reused whenever possible; missing components are installed only for this app. Xcode, Node.js, and Homebrew are not required, and existing tools are never modified." },
  "本功能会检查视频解析、下载及媒体处理所需组件。应用已随附可直接使用的基础组件，也会复用设备上的现有工具；联网更新为可选操作，无需安装 Xcode、Node.js 或 Homebrew。": { "zh-Hant": "此功能會檢查影片解析、下載與媒體處理所需的元件。應用程式已隨附可直接使用的基礎元件，也會使用裝置上的現有工具；連線更新為選用操作，無需安裝 Xcode、Node.js 或 Homebrew。", en: "This checks the components required for video parsing, downloading, and media processing. Ready-to-use baseline components are included, existing device tools can also be reused, and online updates are optional. Xcode, Node.js, and Homebrew are not required." },
  "待检查": { "zh-Hant": "待檢查", en: "Not checked" },
  "建议更新": { "zh-Hant": "建議更新", en: "Update recommended" },
  "当前可用，建议更新": { "zh-Hant": "目前可用，建議更新", en: "Ready · Update available" },
  "已就绪": { "zh-Hant": "已就緒", en: "Ready" },
  "需配置": { "zh-Hant": "需設定", en: "Setup required" },
  "需要配置": { "zh-Hant": "需要設定", en: "Setup required" },
  "组件将保存到当前用户的应用支持目录，不需要管理员权限。": { "zh-Hant": "元件將儲存至目前使用者的應用程式支援資料夾，不需要管理員權限。", en: "Components are stored in the current user’s Application Support folder and do not require administrator access." },
  ...
  "✅当前设备环境齐全，可直接使用": { "zh-Hant": "✅此裝置的環境已齊備，可直接使用", en: "✅This device is ready to use." },
  "检查本设备，以准备按需配置": { "zh-Hant": "檢查此裝置，以準備按需設定", en: "Check This Device" },
  "一键安装/更新": { "zh-Hant": "一鍵安裝/更新", en: "Install/Update" },
  "更新组件（可选）": { "zh-Hant": "更新元件（選用）", en: "Update Component (Optional)" },
  ...
  "自动配置需要联网；下载支持断点续传与最多 3 次重试，安装文件通过 HTTPS 获取并进行 SHA-256 完整性校验。FFmpeg 会按当前 Mac 架构使用主、备发行源。": { "zh-Hant": "自動設定需要網路連線；下載支援斷點續傳與最多 3 次重試。安裝檔案透過 HTTPS 取得，並進行 SHA-256 完整性驗證。FFmpeg 會根據目前 Mac 架構使用主要與備用發佈來源。", en: "Automatic setup requires internet access. Downloads support resuming and up to three retries. Installation files are obtained over HTTPS and verified with SHA-256. FFmpeg uses primary and fallback distribution sources for the current Mac architecture." },
  "应用随附基础组件，无需联网即可满足首次使用；仅可选更新需要联网。下载支持断点续传与重试，并执行 SHA-256 完整性校验。": { "zh-Hant": "應用程式隨附基礎元件，首次使用無需連線；只有選用更新需要網路。下載支援續傳與重試，並執行 SHA-256 完整性驗證。", en: "Baseline components are included for offline first use. Only optional updates require internet access. Downloads support resuming and retries and are verified with SHA-256." },
  ...
  "用于媒体检查、合并与转换，主源为": { "zh-Hant": "用於媒體檢查、合併與轉換，主要來源為", en: " inspects, merges, and converts media. Its primary source is " },
  "备用源为": { "zh-Hant": "備用來源為", en: "; fallback sources are " },
  "与": { "zh-Hant": "與", en: " and " },
  "实际适用 LGPL/GPL 条款以所安装构建为准。": { "zh-Hant": "實際適用的 LGPL/GPL 條款以已安裝的建置版本為準。", en: ". Applicable LGPL/GPL terms depend on the installed build." },
  "macOS 13 或更高版本": { "zh-Hant": "macOS 13 或更新版本", en: "macOS 13 or Later" },
