import { windowsAdapters } from "../platforms/windows/adapters";
import type {
  DownloadMode,
...
export async function openPreferences(): Promise<void> {
  ...
  if (!nativeBridge) {
    throw new Error("偏好设置仅可在 EK StreamDL 桌面应用中打开");
  }
...
export async function parseVideo(inputText: string): Promise<VideoMetadata> {
  ...
  const adapter = windowsAdapters.find((candidate) => candidate.match(normalizedUrl));
  if (!adapter) {
...
export async function downloadVideo(
  ...
  const adapter = windowsAdapters.find((candidate) => candidate.match(metadata.normalizedUrl));
