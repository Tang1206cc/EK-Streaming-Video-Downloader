...
const translations: Record<string, Translation> = {
  ...
  "本功能会检查视频解析、下载及媒体处理所需组件。应用会优先复用设备上已有工具；缺失时仅安装应用专用副本，无需安装 Python、Node.js 或包管理器，也不会改动已有工具。": { "zh-Hant": "此功能會檢查影片解析、下載與媒體處理所需的元件。應用程式會優先使用裝置上現有的工具；如有缺少，僅會安裝應用程式專用副本，無需安裝 Python、Node.js 或套件管理器，也不會改動現有工具。", en: "This checks the components required for video parsing, downloading, and media processing. Existing tools are reused whenever possible; missing components are installed only for this app. Python, Node.js, and package managers are not required, and existing tools are never modified.", ja: "これにより、ビデオの解析、ダウンロード、メディア処理に必要なコンポーネントがチェックされます。既存のツールは可能な限り再利用されます。不足しているコンポーネントはこのアプリに対してのみインストールされます。 Python、Node.js、およびパッケージ マネージャーは必要なく、既存のツールが変更されることはありません。" },
  "本功能会检查视频解析、下载及媒体处理所需组件。应用已随附可直接使用的基础组件，也会复用设备上的现有工具；联网更新为可选操作，无需安装 Python、Node.js 或包管理器。": { "zh-Hant": "此功能會檢查影片解析、下載與媒體處理所需的元件。應用程式已隨附可直接使用的基礎元件，也會使用裝置上的現有工具；連線更新為選用操作，無需安裝 Python、Node.js 或套件管理器。", en: "This checks the components required for video parsing, downloading, and media processing. Ready-to-use baseline components are included, existing device tools can also be reused, and online updates are optional. Python, Node.js, and package managers are not required.", ja: "これにより、ビデオの解析、ダウンロード、メディア処理に必要なコンポーネントがチェックされます。すぐに使用できるベースライン コンポーネントが含まれており、既存のデバイス ツールも再利用でき、オンライン アップデートはオプションです。 Python、Node.js、およびパッケージ マネージャーは必要ありません。" },
  ...
  "组件将保存到当前用户的本地应用数据目录，不需要管理员权限。": { "zh-Hant": "元件將儲存至目前使用者的本機應用程式資料目錄，不需要管理員權限。", en: "Components are stored in the current user’s local application-data folder and do not require administrator access.", ja: "コンポーネントは現在のユーザーのローカルのアプリケーション データ フォルダーに保存され、管理者アクセスは必要ありません。" },
  ...
  "自动配置需要联网；下载支持断点续传与最多 3 次重试，安装文件通过 HTTPS 获取并进行 SHA-256 完整性校验。FFmpeg 会按 Windows x64 架构使用主、备发行源。": { "zh-Hant": "自動設定需要網路連線；下載支援斷點續傳與最多 3 次重試。安裝檔案透過 HTTPS 取得，並進行 SHA-256 完整性驗證。FFmpeg 會依 Windows x64 架構使用主要與備用發佈來源。", en: "Automatic setup requires internet access. Downloads support resuming and up to three retries. Installation files are obtained over HTTPS and verified with SHA-256. FFmpeg uses primary and fallback distribution sources for Windows x64.", ja: "自動セットアップにはインターネットアクセスが必要です。ダウンロードでは、再開と最大 3 回の再試行がサポートされています。インストール ファイルは HTTPS 経由で取得され、SHA-256 で検証されます。 FFmpeg は、Windows x64 のプライマリおよびフォールバックの配布ソースを使用します。" },
  ...
  "用于媒体检查、合并与转换，Windows x64 主源为": { "zh-Hant": "用於媒體檢查、合併與轉換，Windows x64 主要來源為", en: " inspects, merges, and converts media. The primary Windows x64 source is ", ja: " メディアを検査、結合、変換します。主要な Windows x64 ソースは次のとおりです。 " },
  "备用源为": { "zh-Hant": "備用來源為", en: "; the fallback source is ", ja: ";フォールバックソースは " },
  "实际适用的 GPL 与第三方许可条款以安装包内文件为准。": { "zh-Hant": "實際適用的 GPL 與第三方授權條款以安裝套件內檔案為準。", en: ". The installed GPL build is governed by its bundled third-party licenses.", ja: "。インストールされた GPL ビルドは、バンドルされているサードパーティ ライセンスによって管理されます。" },
  "用于媒体检查、合并与转换；随附的 Windows x64 基础版本为": { "zh-Hant": "用於媒體檢查、合併與轉換；隨附的 Windows x64 基礎版本為", en: " inspects, merges, and converts media. The included Windows x64 baseline is an ", ja: " メディアを検査、結合、変換します。付属の Windows x64 ベースラインは、 " },
  " 构建；可选联网更新仍沿用 ffmpeg-static 与 BtbN 官方发布源，具体许可随实际版本适用。": { "zh-Hant": " 構建；選用的連線更新仍沿用 ffmpeg-static 與 BtbN 官方發佈來源，具體授權依實際版本適用。", en: " build. Optional online updates continue to use the official ffmpeg-static and BtbN release sources; licensing follows the version installed.", ja: " 構築します。オプションのオンライン更新では、引き続き公式の ffmpeg-static および BtbN リリース ソースが使用されます。ライセンスはインストールされているバージョンに従います。" },
  "Windows 10 22H2 或更高版本": { "zh-Hant": "Windows 10 22H2 或更新版本", en: "Windows 10 22H2 or Later", ja: "Windows 10 22H2以降" },
