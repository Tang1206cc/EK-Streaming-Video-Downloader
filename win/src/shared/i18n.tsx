...
const translations: Record<string, Translation> = {
  ...
  "本功能会检查视频解析、下载及媒体处理所需组件。应用已随附可直接使用的基础组件，也会复用设备上的现有工具；联网更新为可选操作，无需安装 Python、Node.js 或包管理器。": { "zh-Hant": "此功能會檢查影片解析、下載與媒體處理所需的元件。應用程式已隨附可直接使用的基礎元件，也會使用裝置上的現有工具；連線更新為選用操作，無需安裝 Python、Node.js 或套件管理器。", en: "This checks the components required for video parsing, downloading, and media processing. Ready-to-use baseline components are included, existing device tools can also be reused, and online updates are optional. Python, Node.js, and package managers are not required." },
  ...
  "用于媒体检查、合并与转换；随附的 Windows x64 基础版本为": { "zh-Hant": "用於媒體檢查、合併與轉換；隨附的 Windows x64 基礎版本為", en: " inspects, merges, and converts media. The included Windows x64 baseline is an " },
  " 构建；可选联网更新仍沿用 ffmpeg-static 与 BtbN 官方发布源，具体许可随实际版本适用。": { "zh-Hant": " 構建；選用的連線更新仍沿用 ffmpeg-static 與 BtbN 官方發佈來源，具體授權依實際版本適用。", en: " build. Optional online updates continue to use the official ffmpeg-static and BtbN release sources; licensing follows the version installed." },
