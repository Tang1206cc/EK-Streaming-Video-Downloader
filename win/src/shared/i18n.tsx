...
const translations: Record<string, Translation> = {
  ...
  "自动配置需要联网；下载支持断点续传与最多 3 次重试，安装文件通过 HTTPS 获取并进行 SHA-256 完整性校验。FFmpeg 使用 Windows x64 官方自动构建。": { "zh-Hant": "自動設定需要網路連線；下載支援斷點續傳與最多 3 次重試。安裝檔案透過 HTTPS 取得，並進行 SHA-256 完整性驗證。FFmpeg 使用 Windows x64 官方自動建置。", en: "Automatic setup requires internet access. Downloads support resuming and up to three retries. Installation files are obtained over HTTPS and verified with SHA-256. FFmpeg uses the official Windows x64 auto-build." },
  ...
  "用于媒体检查、合并与转换，Windows x64 构建来自": { "zh-Hant": "用於媒體檢查、合併與轉換，Windows x64 建置來自", en: " inspects, merges, and converts media. The Windows x64 build is sourced from " },
