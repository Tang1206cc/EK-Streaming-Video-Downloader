...
function html() {
  ...
  </style></head><body><h1 id="title"></h1><hr><label class="row"><span id="launchLabel"></span><input id="launch" type="checkbox"></label><p class="hint" id="launchHint"></p><div class="row"><span id="themeLabel"></span><div class="segmented" id="theme"></div></div><label class="row"><span id="escLabel"></span><input id="esc" type="checkbox"></label><label class="row"><span id="quitLabel"></span><input id="quit" type="checkbox"></label><div class="row"><span id="languageLabel"></span><select id="language"><option value="zh-Hans">简体中文</option><option value="zh-Hant">繁體中文</option><option value="en">English</option><option value="ja">日本語</option></select></div><div class="spacer"></div><div class="footer"><button class="secondary" id="reset"></button><div class="right"><label><span id="autoLabel"></span> <input id="auto" type="checkbox"></label><div><span class="version" id="version"></span> <button class="primary" id="check"></button></div></div></div><script>
  const words={
    ...
    ja:{title:'環境設定',launch:'ログイン時に起動',launchHint:'現在の Windows ユーザーのサインイン後に自動起動します',theme:'外観',themes:['システム','ライト','ダーク'],esc:'Esc キーで EK StreamDL を終了',quit:'最後のウィンドウを閉じたときに EK StreamDL を終了',language:'言語',reset:'デフォルトに戻す',auto:'アップデートを自動確認',version:'現在のバージョン',check:'アップデートを確認'}
